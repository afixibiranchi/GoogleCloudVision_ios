...
- (IBAction)loadImageButtonTapped:(UIButton *)sender {
	...
	UIAlertController * alert=   [UIAlertController
								  ...
								  message:@"Choose from Camera or PhotoLibrary"
...
- (void)viewDidLoad {
	...
	//self.navigationItem.title = @"Google Cloud Vision";
    self.navigationItem.title = @"Photo Analyser";
...
-(void)viewWillLayoutSubviews{
  ...
  CGRect labelFrame		  = self.labelResults.frame;
  labelFrame.origin.y	  = imageViewFrame.origin.y + imageViewFrame.size.height + 30;
  self.labelResults.frame = labelFrame;
  
  NSLog(@"labelFrame : %@", NSStringFromCGRect(imageViewFrame));

  
  CGRect faceResultsFrame = self.faceResults.frame;
  faceResultsFrame.origin.y	= labelFrame.origin.y + labelFrame.size.height + 30;
  self.faceResults.frame  = faceResultsFrame;
  
  [self.view updateConstraints];
  

  
  NSLog(@"faceResultsFrame : %@", NSStringFromCGRect(imageViewFrame));
   */
